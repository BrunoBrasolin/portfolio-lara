...
import type { NavigationData, NavLink } from "@/lib/sanity"

interface NavigationProps {
  data?: NavigationData | null
}

// Fallback nav links when Sanity is not configured
...
export function Navigation({ data }: NavigationProps) {
  ...
  const navLinks = content.navLinks || fallbackNavLinks
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 lg:h-20">
          <a
            href="#"
            className={cn(
              "font-serif text-xl lg:text-2xl font-bold tracking-tight text-foreground",
          ...
          {/* Mobile Menu Button */}
          <button
            className={cn("lg:hidden p-2 text-foreground", isScrolled ? "text-black" : "text-white")}
          ...
          <div className="px-4 py-4 space-y-2">
            ...
            <div className="pt-2">
              <Button asChild className="w-full">
                <a href={content.ctaLink} onClick={() => setIsOpen(false)}>
                  {content.ctaText}
                </a>
              </Button>
            </div>
